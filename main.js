(function () {
    ...
    // bot toggle
    const botContainer = document.createElement("div"); botContainer.className = "flex items-center gap-3";
    const botToggle = document.createElement("input"); botToggle.type = "checkbox"; botToggle.id = "playBot"; botToggle.className = "w-6 h-6 accent-blue-500";
    const botLabel = document.createElement("label"); botLabel.htmlFor = "playBot"; botLabel.textContent = "Playing vs Bot?"; botLabel.className = "text-base text-gray-700 dark:text-gray-300 font-medium";
    botContainer.append(botToggle, botLabel);
    field.appendChild(botContainer);

    // custom message toggle
    const customContainer = document.createElement("div"); customContainer.className = "flex items-center gap-3";
    const customToggle = document.createElement("input"); customToggle.type = "checkbox"; customToggle.id = "customMsg"; customToggle.className = "w-6 h-6 accent-green-500";
    const customLabel = document.createElement("label"); customLabel.htmlFor = "customMsg"; customLabel.textContent = "Enable custom messages?"; customLabel.className = "text-base text-gray-700 dark:text-gray-300 font-medium";
    customContainer.append(customToggle, customLabel);
    field.appendChild(customContainer);

    // color select
    const colorLabel = document.createElement("label"); colorLabel.textContent = "Your color:"; colorLabel.className = "text-base text-gray-700 dark:text-gray-300 font-medium";
    const colorSelect = document.createElement("select"); colorSelect.className = "w-full px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400";
    ["White","Black"].forEach(color => { const o = document.createElement("option"); o.value = color.toLowerCase()[0]; o.textContent = color; colorSelect.appendChild(o); });
    field.append(colorLabel, colorSelect);

    // think time input
    const timeLabel = document.createElement("label"); timeLabel.textContent = "Think time (sec):"; timeLabel.className = "text-base text-gray-700 dark:text-gray-300 font-medium";
    const timeInput = document.createElement("input"); timeInput.type = "number"; timeInput.value = 30; timeInput.min = 1; timeInput.max = 300;
    ...
    // footer
    const credit = document.createElement("p"); credit.textContent = "Made by Takaso";
    ...
    header.addEventListener("mousedown", e => {
        dragging = true; const rect = GUI.getBoundingClientRect(); offX = e.clientX - rect.left; offY = e.clientY - rect.top;
    });
    ...
    function blockFor(seconds) {
        ...
        Object.assign(overlay.style, { position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0, 0, 0, 0)", zIndex: 9998, pointerEvents: "all" });
        ...
        intervalId = setInterval(() => {
            ...
            if (t <= 0) {
                ...
                overlay.remove(); countdown.remove(); msgBox.remove();
